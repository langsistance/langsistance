...
1. 干燥空气查询（聊天路径）：回答含 5-10 篇切题专利（标题/申请号/申请人 + 相关性分析），无跨领域噪声（不得出现放射治疗/显示器/氢能等）
2. 用户追问某篇 → agent 调 fetch_patent_spec → 回答基于真实提炼内容（发明点/权利要求要点），不编造
3. 前端部署后：执行中显示步骤行，完成后折叠为「⏱ 已用时间 · N 步」
4. 全量回归零新增失败；长任务管线下载行为不变
