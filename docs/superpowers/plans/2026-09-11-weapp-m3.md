...
在 `frontend/weapp/src/pages/chat/index.scss` 末尾追加：

```scss
@import '../../components/PrivacyPopup/index.scss';
```

（Taro 会把组件 scss 随组件 import 打包，此处不重复引入——**若 `npm run build:weapp` 报重复样式，删掉这一行，改为在 `index.tsx` 的 `import PrivacyPopup from ...` 之后加 `import '../../components/PrivacyPopup/index.scss'`**。）
...
在 `frontend/weapp/src/pages/chat/index.scss` 里追加样式引入：

```scss
@import '../../components/AttachmentBar/index.scss';
```
...
```scss
@import '../../components/LongTaskCard/index.scss';
```
