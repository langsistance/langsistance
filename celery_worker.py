...
async def _run_pipeline(
    ...
    if not is_file_upload_mode and not is_direct_id_mode and not patent_ids:
        ...
        if combined_text and len(combined_text) > 20:
            ...
            EXTRACT_ALL_PROMPT = (
                ...
                "'17429113��18012525��18331482������3��ר�������'). "
                "When the user mentions 'ר�������', 'application number', "
                "or 'patent application', the adjacent numbers ARE the IDs. "
                "\n\n"
                "���� HOW TO IDENTIFY THE SOURCE COUNTRY ����\n"
                "CNIPA (China / �й�����֪ʶ��Ȩ��) application numbers:\n"
                ...
                "USPTO (United States / ����ר���̱��) application numbers:\n"
            ...
            EXTRACT_ONE_PROMPT = (
                ...
                "e.g. 18331482). "
                "Look for numbers labeled '�����' or 'Application Number', "
    ...
    if not patent_ids and scene_candidates:
        ...
        if patent_ids:
            ...
            update_task_status(task_id, 'searching_patents', 5,
                               f'������ {len(patent_ids)} ��ר������ʼ����',
                               patent_ids=patent_ids)
        else:
            set_task_failed(task_id, 'δ�ҵ�ƥ���ר��')
            user_id_for_analytics = params.get('user_id', '')
            if user_id_for_analytics:
                track_event("long_task:fail", user_id=user_id_for_analytics,
                            task_id=task_id,
    ...
    for i, patent_id in enumerate(pending):
        ...
        _result = _handle_task_pause(task_id, _uid, _completed, total, {
            'completed': [r.get('ר����', '') for r in table_rows if not r.get('_failed')],
            'current': patent_id,
            'pending': pending[i:],
            'completed_rows': table_rows,
            'failed': [r.get('ר����', '') for r in table_rows if r.get('_failed')],
            'columns': columns,
        })
        if _result:
            return _result
        ...
        try:
            ...
            update_task_status(task_id, 'analyzing',
                               progress_pct(completed_before + i, total),
                               f'���ڷ�����{patent_index}/{total}����{patent_id}',
                               table_rows=table_rows)
        ...
        save_checkpoint(task_id, {
            'completed': [r.get('ר����', patent_id) for r in table_rows if not r.get('_failed')],
            'current': patent_id,
            'pending': pending[i+1:],
            'completed_rows': table_rows,
            'failed': [r.get('ר����', patent_id) for r in table_rows if r.get('_failed')],
            'columns': columns,
        })

        update_task_status(task_id, 'analyzing',
                           progress_pct(completed_before + i + 1, total),
                           f'����� {len(table_rows)}/{total} ��ר������',
                           table_rows=table_rows)
    ...
    summary_updater = ThrottledSummaryUpdater(
        task_id, progress=76, step_msg='����׫дִ��ժҪ...',
    )
    report_title = 'ר����������' if batch_lang == 'zh' else 'Patent Analysis Report'
    ...
    def _assemble_report(
        ...
        if exec_summary:
            exec_heading = 'ִ��ժҪ' if batch_lang == 'zh' else 'Executive Summary'
    ...
    report_parts = []
    sections = outline.get('sections', [{'heading': '�������', 'description': ''}])
    for idx, section in enumerate(sections):
        sec_pct = 80 + int((idx + 1) / len(sections) * 10)
        step_msg = f'����׫д��{section["heading"]}'
...
def execute_prosecution_analysis(self, task_id: str, params: dict):
    ...
    async def _run():
        ...
        if not documents:
            if lang == 'zh':
                msg = f"USPTOδ����ר������ {app_number} ���κ��ļ�������ԭ������Ų����ڡ���Ȩ���ʡ�����δ������"
            else:
        ...
        if not docs_to_download:
            if lang == 'zh':
                msg = "δ�ҵ��ɷ���������ļ����� Office Action��Response �� Amendment�������ܸ�ר����δ����ʵ�����׶Ρ�"
            else:
        ...
        for _i, _doc in enumerate(docs_to_download):
            ...
            update_task_status(
                ...
                f'���ڷ�����{doc_index}/{total_dl}����{_doc.description[:40]}'
                if lang == 'zh'
        ...
        summary_updater = ThrottledSummaryUpdater(
            task_id,
            progress=80,
            step_msg='����׫дִ��ժҪ...' if lang == 'zh' else 'Writing executive summary...',
        )
        ...
        update_task_status(task_id, 'generating_report', 90,
                           '����׫д���' if lang == 'zh' else 'Report writing complete',
        ...
        if flash_provider and len(documents) > 1:
            ...
            try:
                selection = await flash_provider.complete_json(
                    ...
                    "(˵����). The specification is typically:\n"
...
async def export_docx_async(report_text: str, table_rows: list, columns: list) -> bytes:
    ...
    LATIN_FONT = 'Arial'
    CJK_FONT = 'Microsoft YaHei'  # ΢���ź�, pre-installed on Windows
    FALLBACK_FONT = 'SimSun'      # ����, universal fallback
