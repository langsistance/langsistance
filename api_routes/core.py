...
async def _classify_long_task_async(
    ...
    system_prompt = (
        ...
        "2. families — 用户要分析专利的**同族专利**在各国的审查过程。"
        "关键词：「各国」、「同族」、「跨国」、「各国家」、「全球」、「全球族」、"
        "「同族审查」、「各国审查」、「审查历史」+「族/各国/全球」、"
        "「family」、「jurisdiction」。\n"
        "   典型表达：「帮我看看 XX专利在各国的审查过程」\n"
        "   「分析 XX专利的全球同族审查情况」\n"
        "   - patent_ids 中只提取 **1个** 专利号\n"
        "   - patent_source 填该专利所属的专利局\n"
        "   - **入口号可以是任何国别** —— 中/美/欧/日号都可以走本场景；"
        "系统会自己用 EPO 族解析出各国成员。不要因为号码不是美国号就改判 direct_ids。\n"
        "   例：「帮我看看 US12506212 在各国的审查过程」→ scenario: families, patent_ids: [\"US12506212\"]\n"
        "   例：「CN114601976A全球专利族审查历史分析」→ scenario: families, patent_ids: [\"CN114601976A\"], patent_source: cnipa\n"
        "   例：「分析 EP4000000 的全球同族审查差异」→ scenario: families, patent_ids: [\"EP4000000\"], patent_source: epo\n"
        "   - 与 china_prosecution 的边界：问「这件中国专利在**中国**经历了什么审查」"
        "（复审/无效/异议）→ china_prosecution；"
        "问它「在**各国/全球**的审查过程与差异」→ families。\n"
        "3. direct_ids — 用户提问本身包含专利申请号，或者用户提出了一个全新独立的问题。\n"
        ...
        "4. conversation_refs — 用户的问题是针对历史对话中出现的专利结果的追问，"
        "不看历史对话就无法确定要分析哪些专利。\n"
        "   - patent_ids 从历史对话中提取\n"
                ...
                if isinstance(openai_agent, dict) and openai_agent.get('intent') == 'long_task':
                    ...
                    try:
                        ...
                        if queue_result == 'running':
                            ...
                            track_event("long_task:submit", user_id=str(local_user_id),
                                        task_id=task_id,
                                        patent_count=1 if is_single_patent else len(patent_ids),
                                        patent_source=patent_source,
                                        session_id=session_id or None,
                                        query_text=request.query)
                            conn.close()
                        ...
                        if is_families:
                            status_msg = (
                                '同族专利跨国审查分析任务已提交' if queue_result == 'running'
                                else '同族专利跨国审查分析任务已排队，将在当前任务完成后自动开始'
                            )
                        elif is_prosecution:
                            status_msg = (
