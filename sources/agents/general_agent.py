        ...
        if original_total <= SMALL_LIST_THRESHOLD:
            ...
            if len(batch_json) <= MAX_BATCH_JSON_CHARS_FOR_LLM:
                ...
                faithful_system_prompt = (
                    "You are a data presenter. Format the input data as readable, "
                    "well-structured Markdown.\n\n"
                    "CRITICAL RULES:\n"
                    "1. Output EVERY field from EVERY item — do NOT skip or omit any field. "
                    "The required-fields checklist below is your contract.\n"
                    "2. Format for READABILITY: use headings, sections, bullet lists, "
                    "and comparison tables where they improve clarity.\n"
                    "3. Nested objects: expand them fully — do NOT dump raw JSON strings. "
                    "Present their contents with proper formatting (sub-sections or indented lists).\n"
                    "4. Arrays of objects: present each one individually with ALL fields. "
                    "A comparison table is acceptable ONLY IF every field is included.\n"
                    "5. Simple key-value metadata: group them concisely at the top.\n"
                    "6. Every URL must be copied exactly and verbatim.\n"
                    "7. Image URLs MUST use ![alt](URL) syntax for inline display.\n"
                    "8. Add brief section headings for structure, but do NOT write "
                    "a summary or conclusion — let the data speak."
                )
                faithful_user_content = (
                    f"REQUIRED FIELDS — verify that EVERY one of these {len(first_keys)} fields "
                    f"appears in your output for each of the {len(pruned)} items:\n\n"
                    f"{field_checklist}\n\n"
                    f"ANY FIELD MISSING FROM YOUR OUTPUT IS AN ERROR.\n\n"
                    f"Format this data readably while keeping ALL fields:\n\n"
                    f"{url_checklist}"
                    f"{batch_json}"
                )
                try:
