...
# 末尾一律用 `(?!\d)` 而非 `\b`。**Python 3 里 CJK 字符也算 word character**，
# 所以 `CN114601976A全球…` 的 `A全` 之间、`CN114601976的…` 的 `6的` 之间都没有
# 词边界 —— 而"号紧挨着中文"正是中文用户最典型的写法。2026-09-20 生产：
# `CN114601976A全球专利族审查历史分析` 被判"未包含专利号"，深度分析任务被拒
# 三次。`(?!\d)` 表达的是真正想要的约束（号码不要继续接数字），与后随字符是
# 中文、标点还是行尾无关。
_PATENT_ID_PATTERNS = (
    re.compile(r"(?<![\dA-Za-z])\d{8}(?!\d)"),        # US 8 位申请/授权号
    re.compile(r"(?<!\d)\d{2}/\d{6}(?!\d)"),          # US 回执号 30/076,484
    re.compile(r"(?<![A-Za-z])US\d{6,}(?!\d)", re.I),  # US30076484
    re.compile(r"(?<!\d)20[12]\d{8,9}(?:\.\d)?(?!\d)"),  # CN 申请号
    re.compile(r"(?<![A-Za-z])CN\d{7,12}[A-Z]?\d?(?!\d)", re.I),  # CN 公开号
)
