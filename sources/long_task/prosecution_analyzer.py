...
async def generate_executive_summary(
    ...
    if lang == "zh":
        system_prompt = (
            ...
            "- Before: [原始范围]\n"
            "- After: [修改后范围]\n"
            "- Why it worked: [这个修改为什么绕开了 prior art]\n"
            "- 用 2-4 个子弹点，清晰对比修改前后的范围变化\n\n"
            "### 4. Key Prosecution Insights\n\n"
            "对无效/FTO/Claim 解释的实用洞察：\n"
            "- Potential invalidity challenge points: [哪些限制是有效的关键？需要什么 prior art 才能攻击？]\n"
            "- FTO considerations: [哪些限制在侵权分析中最重要？]\n"
            "- Prosecution history estoppel risks: [审查中哪些陈述可能限制 Claim 范围？]\n"
            "- 用 3-5 个子弹点\n\n"
            "══════════════════════════════════════\n"
            "CRITICAL 法律语气规则\n"
            "══════════════════════════════════════\n\n"
        ...
        user_content = (
            ...
            f"撰写「核心审查洞察」。要求：\n"
            f"- 子弹点格式（- 开头），不是段落\n"
            f"- 4 个小节：Why Allowed / Strongest Rejection / Key Amendment / Prosecution Insights\n"
            f"- 800-1200 字，不重复，不介绍背景\n"
            f"- 对审查员意图使用「Based on the record」「likely」「suggests」，不说「admitted」「认可」"
        )
    else:
        system_prompt = (
        ...
        user_content = (
            ...
            f"Write the 'Key Prosecution Insights' section. Requirements:\n"
