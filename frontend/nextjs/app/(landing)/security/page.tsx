...
const COPY = {
  zh: {
    ...
    logsTitle: '日志与长任务',
    logsBody: '用于诊断的日志中，提问内容仅保留前 80 个字符。当您发起长时间分析任务时，任务记录会保存该任务的提问与参数，以便连接中断后恢复；这些记录随账号数据一同删除，您也可以随时联系我们删除。',
  ...
  en: {
    ...
    logsTitle: 'Logging and long-running tasks',
    logsBody: 'In diagnostic logs, prompt content is truncated to the first 80 characters. When you start a long-running analysis task, the task record stores that task\'s prompt and parameters so it can resume after a disconnection; these records are removed together with your account data, and you may contact us to delete them at any time.',
