...
const BLOCK_STYLE = {
    ...
    li: 'font-size:32rpx;line-height:1.75;letter-spacing:1rpx;',
    blockquote: 'padding-left:44rpx;border-left:8rpx solid #e1e4e8;color:#6e7781;',
    pre: 'padding:36rpx;background:#f6f8fa;border-radius:24rpx;font-size:28rpx;line-height:1.7;',
...
function decorate(html) {
    return html.replace(/<(p|h1|h2|h3|h4|li|ul|ol|blockquote|pre|code)(\s[^>]*)?>/g, (whole, tag, attrs = '') => {
        const style = BLOCK_STYLE[tag];
        // 已有内联样式（如代码块内的 span）不覆盖
        if (!style || /style\s*=/.test(attrs))
            return whole;
        return `<${tag}${attrs} style="${style}">`;
    });
}
function renderHtml(md) {
    return decorate(marked_1.marked.parse(md, { async: false }));
}
/**
