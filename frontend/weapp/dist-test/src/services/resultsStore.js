...
exports.resultsStore = exports.MAX_MEMORY_SETS = exports.MAX_INDEX_ENTRIES = exports.MAX_PERSISTED_SETS = exports.STORAGE_KEY = void 0;
exports.createResultsStore = createResultsStore;
...
exports.STORAGE_KEY = 'copiioai_results';
/**
 * **落盘那层**保留的集数上限。小程序总量 10MB，20 集 × 约 40KB ≈ 800KB，留足余量。web 是 100。
 */
exports.MAX_PERSISTED_SETS = 20;
/** index 条目上限。与集数同量级即可。web 是 200。 */
exports.MAX_INDEX_ENTRIES = 40;
/**
...
function createResultsStore(storage) {
    ...
    function evictMemory() {
        while (mem.size > exports.MAX_MEMORY_SETS) {
            let oldestId = '';
    ...
    function read() {
        try {
            const raw = storage.getSync(exports.STORAGE_KEY);
    ...
    return {
        ...
        persist(payload, meta) {
            ...
            try {
                const shape = read();
                shape.sets[payload.setId] = (0, results_1.pruneResults)(payload);
                shape.index = [
                ...
                const byOldest = Object.entries(shape.sets).sort((a, b) => (findSavedAt(shape, a[0]) - findSavedAt(shape, b[0])));
                for (const [id] of byOldest.slice(0, Math.max(0, byOldest.length - exports.MAX_PERSISTED_SETS))) {
                    delete shape.sets[id];
                }
                shape.index = shape.index
                    .sort((a, b) => b.savedAt - a.savedAt)
                    .slice(0, exports.MAX_INDEX_ENTRIES);
                storage.setSync(exports.STORAGE_KEY, shape);
            }
            catch {
...
function findSavedAt(shape, setId) {
    const entry = shape.index.find((e) => e.setId === setId);
    return entry ? entry.savedAt : 0;
}
/** 默认实例：接到 Taro 的同步存储。 */
exports.resultsStore = createResultsStore({
    getSync: (key) => taro_1.default.getStorageSync(key),
    setSync: (key, value) => taro_1.default.setStorageSync(key, value),
});
//# sourceMappingURL=resultsStore.js.map